...
async fn main() -> Result<()> {
    ...
    if job_config.job_name != "semantic_scoring" {
        return Err(anyhow::anyhow!(
            "Invalid job_name: expected 'semantic_scoring', got '{}'",
            job_config.job_name
        ));
    }
    ...
    // Load model weights
    let model_weights_path = format!("{model_path}/model.safetensors");
    let vb =
        unsafe { VarBuilder::from_mmaped_safetensors(&[model_weights_path], DType::F32, &device)? };
    let model = BertModel::load(vb, &model_config)?;

    // Compute semantic scores
    let scores = semantic_scores(
        &model,
        &tokenizer,
        &job_config.value,
        &job_config.values,
